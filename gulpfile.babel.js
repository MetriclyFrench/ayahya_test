...
gulp.task('build', () => {
    runSequence(['sass', 'js', 'pluginsJS', 'fonts', 'images'], 'hugo')
})

gulp.task('build-preview', () => {
    runSequence(['sass', 'js', 'pluginsJS', 'fonts', 'images'], 'hugo-preview')
})
...
gulp.task('pluginsJS', () => {
    ...
    .pipe($.plumber({ errorHandler: onError }))
    .pipe($.newer('static/js/plugins/*.js'))
    .pipe($.print())
    .pipe($.babel())
